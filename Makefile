...
bin/$(1): $(patsubst %,$(2)/obj/%.d,$(basename $($(1)_SOURCES))) $(patsubst %,$(2)/obj/%_asm.o,$(basename $($(1)_SOURCESASM))) $(foreach library,$($(1)_LIBRARIES),lib/$(library))
...
swiftos.mkiosfs: swiftos.writebootloader
...
swiftos.writeStages: bin/bootstrap
	@dd if=$^ of=swiftos.iso obs=2048 seek=1 count=6 conv=notrunc status=none
